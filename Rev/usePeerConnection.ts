...
const usePeerConnection = (username: string) => {
    ...
    const tryPeerConnection = useCallback((peerId: string) => {
        ...
        newPeer.on('error', (error: any) => {
            console.error(`âŒ Peer error:`, error);
        ...
        newPeer.on('connection', (conn: any) => {
            console.log('ðŸ“ž HOST received incoming connection from GUEST:', conn.peer);
    ...
    const joinRoom = useCallback((roomId: string) => {
        ...
        newPeer.on('open', (id: string) => {
            ...
            setTimeout(() => {
                try {
                    ...
                    conn.on('error', (error: any) => {
                        console.error('âŒ GUEST connection error:', error);
                    ...
                    console.error('âŒ GUEST failed to initiate connection:', error);
        ...
        newPeer.on('error', (error: any) => {
            console.error('âŒ GUEST peer initialization error:', error);
